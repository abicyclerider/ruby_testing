...
describe NumberGame do
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        second_invalid = '99'
        valid_input = '4'
        allow(game_loop).to receive(:player_input).and_return(first_invalid, second_invalid, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      count = 77
      subject(:game) { described_class.new(5, '5', count) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "That was hard. It took you #{count} guesses!\n"
        expect { game.final_message }.to output(congrats_phrase).to_stdout
      end
    end
  end
