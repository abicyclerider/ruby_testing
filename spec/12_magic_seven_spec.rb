...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when random number is 15 and number is 10' do
      subject(:fixed_random_number) { described_class.new(15) }
      it 'return -5' do
        result = fixed_random_number.subtract_random_number(10)
  ...
  describe '#play' do
    context 'when the random number is 17' do
      subject(:random_number_17) { described_class.new(17) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = random_number_17.play
    ...
    context 'when the random number is 1' do
      # remove the 'x' before running this test
      subject(:random_number_1) { described_class.new(1) }
      it 'will return 7' do
        result = random_number_1.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 10000' do
      # remove the 'x' before running this test
      subject(:random_number_1000) { described_class.new(1000) }
      it 'will return 7' do
        result = random_number_1000.play
        expect(result).to eq(7)
      end
    end
  end
