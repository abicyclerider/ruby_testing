...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:number_end) { double('random_number', value: 8) }
      subject(:game_end) { described_class.new(0, 9, number_end, 8) }
    ...
    context 'when guess and random_number are not equal' do
      let(:number_continue) { double('random_number', value: 8) }
      subject(:game_continue) { described_class.new(0, 9, number_continue, 3) } 
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      subject(:game_less) { described_class.new(0, 9, number_range, 4) }

      before do
        game_less.update_range
      end
      it 'updates min to 5' do
        expect(game_less.min).to eq(5)
      end

      it 'does not update max' do
        expect(game_less.max).to eq(9)
      end
